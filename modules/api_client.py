...
class APIClient:
    def __init__(self, logger):
        ...
        if not all([self.jira_url, self.jira_email, self.jira_token]):
            raise ValueError("JIRA_URL, JIRA_EMAIL, and JIRA_TOKEN must be set in environment variables.")
        self.jira_headers = {
    ...
    def send_request(self, request_to ,endpoint, method, data=None, files=None, params=None):
        ...
        try:
            ...
            self.logger.info(f"Request to {url} successful with status code {response.status_code}.")
